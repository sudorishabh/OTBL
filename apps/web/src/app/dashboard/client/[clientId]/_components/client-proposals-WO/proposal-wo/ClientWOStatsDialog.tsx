      ...
      <div className="space-y-6 pb-6 pt-2 h-[60vh] overflow-y-auto pr-1">
        {allWOs.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-10 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No work orders available</p>
            <p className="text-xs text-slate-400">There are no ongoing or completed work orders to display stats for.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {allWOs.map((wo) => (
              <WOStatRow key={wo.id} wo={wo} />
            ))}
          </div>
        )}
      </div>
