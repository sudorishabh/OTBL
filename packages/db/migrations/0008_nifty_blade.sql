...
--> statement-breakpoint
ALTER TABLE `users` MODIFY COLUMN `role` varchar(50) NOT NULL DEFAULT 'operator';--> statement-breakpoint
...
ALTER TABLE `work_order_site_expenses` ADD CONSTRAINT `work_order_site_expenses_work_order_site_id_work_order_sites_id_fk` FOREIGN KEY (`work_order_site_id`) REFERENCES `work_order_sites`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `work_order_site_expenses` ADD CONSTRAINT `work_order_site_expenses_contractor_id_contractors_id_fk` FOREIGN KEY (`contractor_id`) REFERENCES `contractors`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `work_order_site_expenses` ADD CONSTRAINT `work_order_site_expenses_created_by_users_id_fk` FOREIGN KEY (`created_by`) REFERENCES `users`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
