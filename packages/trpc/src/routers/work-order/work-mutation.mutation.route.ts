...
export const workOrderMutationRouter = router({
  /**
   * Create a new work order with sites - Manager or higher
   */
  createWorkOrder: protectedProcedure.input(createWorkOrderSchema).mutation(
  ...
  /**
   * Edit an existing work order - Manager or higher
   */
  editWorkOrder: protectedProcedure.input(editWorkOrderSchema).mutation(
    handleMutation(async ({ input, ctx }) => {
