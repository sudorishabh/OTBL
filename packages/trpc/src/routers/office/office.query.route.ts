...
export const officeQueryRouter = router({
  ...
  getOffices: protectedProcedure.input(getOfficesSchema).query(
  ...
  getOfficeWorkOrders: publicProcedure.input(getOfficeWorkOrderScema).query(
    handleQuery(async ({ input, ctx }) => {
  ...
  getOfficeStats: publicProcedure.input(getOfficeStatsSchema).query(
    handleQuery(async ({ input, ctx }) => {
  ...
  getOfficeUsers: publicProcedure.input(getOfficeUsersSchema).query(
    handleQuery(async ({ input, ctx }) => {
      const users = await ctx.db
        .select({
          id: userTable.id,
